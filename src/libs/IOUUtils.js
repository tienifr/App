...
function getCurrencyUnits(currency) {
    return 10 ** (currencyDigitsData[currency] || 2);
}
 ...
 * @returns {Number}
 */
function calculateAmount(participants, total, currency, isDefaultUser = false, convertTo2DigitsFormat = false) {
    ...
    if (!isDefaultUser) {
        if (convertTo2DigitsFormat) {
            return amountPerPerson * 100 / getCurrencyUnits(currency);
        }
    ...
    if (convertTo2DigitsFormat) {
        return finalAmount * 100 / getCurrencyUnits(currency);
    }
    return finalAmount;
}
